 ...
 * ```
 */
export const Container = React.forwardRef<CollapsibleRef, CollapsibleProps>(
  (
    {
      initialTabName,
    ...
    const tabBarHeight = useSharedValue<number | undefined>(initialTabBarHeight)
    ...
    const propagateTabChange = (change: IndexChangeEventData<TabName>) => {
      onTabChange?.(change)
    ...
    const getTabBarHeight = (event: LayoutChangeEvent) => {
      const height = event.nativeEvent.layout.height
