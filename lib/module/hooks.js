import { Children, useState, useCallback, useContext, useEffect } from 'react';
import { StyleSheet } from 'react-native';
import { cancelAnimation, useAnimatedReaction, useAnimatedRef, useAnimatedScrollHandler, useSharedValue, withDelay, withTiming, interpolate, Extrapolate, runOnJS, runOnUI, useDerivedValue, useEvent, useHandler } from 'react-native-reanimated';
...
export function useTabsContext(raw) {
    ...
    onScroll: event => {
      ...
      if (focusedTab.value === name) {
        if (IS_IOS) {
          ...
          scrollYCurrent.value = allowHeaderOverscroll ? y : interpolate(y, [0, clampMax], [0, clampMax], Extrapolate.CLAMP);
        } else {
          const {
            y
          } = event.contentOffset;
...
export function useFocusedTab(raw) {
